...
driver = webdriver.Chrome("C:/Users/samue/OneDrive/Área de Trabalho/chromedriver")
...
def get_bl():
    driver.get("https://www.teleco.com.br/blarga1.asp")

    print(driver.execute_script("return window.name"))

    # get the states dropdown
    states_select = Select(driver.find_element_by_id('estado_blf'))
    ...
    for _state in states_select.options:
        ...
        states_select.select_by_visible_text(state)
        sleep(1) # wait for the site to update
        county_select = Select(driver.find_element_by_id('cidades_blf'))
        for i in range(len(county_select.options)):
            # loop through every county
            try:
                count_text = county_select.options[i].text
        ...
        states_dict
        #states_select.select_by_value(state.text)
    with open("teleco_bl.json", "w", encoding="utf8") as f:
        json.dump(states_dict, f, ensure_ascii=False, indent=4)
    return states_dict

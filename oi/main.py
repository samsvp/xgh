...
#%%
df = pd.read_excel("OI TV (15).xls", None)

# First 3 sheets don't have valuable information
for k in list(df.keys())[0:3]:
    del df[k]

# dict with all hotels cnpj
data = {hotel: list(frame.iloc[:,0].dropna())
     for hotel, frame in df.items()}

if not all([len(dt) for _, dt in data.items()]):
    print("WARNING: missing items")
